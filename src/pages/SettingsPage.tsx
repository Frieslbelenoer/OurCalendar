...
export const SettingsPage: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setMessage({ type: 'success', text: 'Profil aman bos!' });
        } catch (error) {
            console.error("Error updating profile:", error);
            setMessage({ type: 'error', text: 'Gagal update nih, sinyal lu jelek?' });
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="p-8 h-full overflow-y-auto w-full animate-fade-in custom-scrollbar">
            <div className="max-w-3xl mx-auto space-y-8 pb-12">
                <header className="mb-10">
                    <h1 className="text-3xl font-bold mb-2 text-white tracking-tight">Pengaturan</h1>
                    <p className="text-secondary text-lg font-light">Atur akun lo biar makin kece.</p>
                </header>

                {/* Profile Section */}
                <section className="space-y-4">
                    <h2 className="text-lg font-semibold text-gray-300 uppercase tracking-wider text-xs mb-4 px-1">Info Profil</h2>
                            ...
                            <div className="flex flex-col items-center gap-4">
                                <div className="relative group">
                                    ...
                                    <div className="absolute inset-0 rounded-full flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 cursor-pointer backdrop-blur-sm">
                                        <span className="text-xs font-medium text-white bg-white/10 px-3 py-1 rounded-full border border-white/20">Ganti</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 w-full space-y-6">
                                <div className="grid grid-cols-1 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-400">Nama Panggung</label>
                                        <input
                                            type="text"
                                            className="input"
                                            value={displayName}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-400">Foto Profil</label>
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <div className="flex items-center gap-4 bg-slate-900/40 p-3 rounded-xl border border-slate-700/50">
                                                <p className="text-sm text-gray-400 flex-1">
                                                    Klik "Gacha" sampe nemu muka yang pas.
                                                </p>
                                                <button
                                                    onClick={() => {
                ...
                {/* Preferences Section */}
                <section className="space-y-4">
                    <h2 className="text-lg font-semibold text-gray-300 uppercase tracking-wider text-xs mb-4 px-1">Setelan Aplikasi</h2>
                    <div className="card bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden divide-y divide-slate-700/50">
                        <div className="p-6 flex items-center justify-between group hover:bg-slate-700/20 transition-colors">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 group-hover:text-gray-400 transition-colors">Biar mata nggak sakit</p>
                                </div>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="p-6 flex items-center justify-between group hover:bg-slate-700/20 transition-colors">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 className="font-medium text-white">Notifikasi</h4>
                                    <p className="text-sm text-gray-500 group-hover:text-gray-400 transition-colors">Biar nggak ketinggalan berita</p>
                                </div>
                            </div>
                            <button
                                type="button"
                ...
                {/* Logout Section */}
                <section className="space-y-4 pt-6 border-t border-slate-700/50">
                    <h2 className="text-lg font-semibold text-red-400 uppercase tracking-wider text-xs mb-4 px-1">Zona Bahaya</h2>
                    ...
                    {user?.groupId && (
                        <button
                            onClick={() => {
                                showConfirm(
                                    'Leave Circle',
                                    'Yakin mau ninggalin circle ini? Semua kenangan akan... ya tetep ada sih, tapi lu ga di circle lagi.',
                                    async () => {
                                        closeConfirm();
                                        await leaveGroup();
                                    },
                            ...
                            className="w-full bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 hover:text-orange-400 border border-orange-500/20 rounded-xl p-4 font-bold transition-all flex items-center justify-center gap-2 group mb-4"
                        >
                    ...
                    <button
                        onClick={() => {
                            showConfirm(
                                'Logout',
                                'Yakin mau cabut sob? Nanti kangen loh.',
                                async () => {
                                    closeConfirm();
                                    await logout();
                                },
                        ...
                        className="w-full bg-red-500/10 hover:bg-red-500/20 text-red-500 hover:text-red-400 border border-red-500/20 rounded-xl p-4 font-bold transition-all flex items-center justify-center gap-2 group"
                    >
                        <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="group-hover:-translate-x-1 transition-transform">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                        </svg>
                        Cabut Dari Sini (Logout)
                    </button>
                </section>
            </div>
